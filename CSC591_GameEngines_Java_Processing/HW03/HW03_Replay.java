...
// CLASS DEFINITION
public class HW03_Replay implements HW03_EventObserver {
    ...
    private static final HW03_Time_Loop O_REPLAY_LOOP =             HW03_Time_Loop.getReplayInstance();
    private static final HW03_Time_Loop O_PLAY_LOOP =               HW03_Time_Loop.getPlayInstance();
    ...
    /*********************************************************************************************************
     * FUNCTION:        HW03_Replay Constructor
     *
     * AUTHOR:          attiffan    Aurora T. Tiffany-Davis
     *
    ...
    public HW03_Replay () {
        try {
            ...
            O_EVENT_MANAGER.registerForEvents(HW03_ObjectEvent.EventType.REPLAY, this, false);
        }
    ...
    public static void stepReplayForward () {
        try {
            ...
            HW03_ObjectLocated oGameObjectToMove;
            ConcurrentHashMap<Integer, HW03_ObjectGame> aoGameObjects;
            int nObjectGUID;
            int i;
            ...
            if (O_REPLAY_LOOP.getTime() == 0) {
                ...
                asReplayLogContents = HW03_Logger.getLogContents(true);
            ...
            nPlayLoopTimeToGetTo = 
                ...
                HW03_Time.getTimerStartTime(S_TIMER_RECORDING, "LOOP");
             ...
             *      1541460694937   3662    77  2   GAME_OBJECT_CHANGE  oGameObject HW03_ObjectPlatformV, 17, 212.0, 314.88342, FALSE
             *  Explained:
            ...
            while (true) {
                ...
                if (nReplayArrayIndex >= asReplayLogContents.size()) {
                    ...
                    if (nPlayLoopTime > nPlayLoopTimeToGetTo) {
                        ...
                        oGameObjectToMove = ((HW03_ObjectLocated) HW03_ObjectGame.getObjectByGUID(nObjectGUID));
                         ...
                         */
                        if (oGameObjectToMove instanceof HW03_ObjectRenderable) {
                            ((HW03_ObjectRenderable) oGameObjectToMove).setHiddenFlag(bObjectFlaggedForRemoval);
                        }
            ...
            if (nReplayArrayIndex >= asReplayLogContents.size()) {
                ...
                aoGameObjects = HW03_ObjectGame.getGameObjects();
                for (HW03_ObjectGame oSingleGameObject : aoGameObjects.values()){
                    if (oSingleGameObject.getRemovalFlag() == true) {
                        HW03_ObjectGame.removeObject(oSingleGameObject);
                    }
                }
                ...
                O_EVENT_MANAGER.raiseEventReplay(HW03_Replay.ReplayEventType.REPLAY_END);
    ...
    public synchronized void handleEvent (HW03_ObjectEvent oEventToHandle) {
        try {
            ...
            CopyOnWriteArrayList<HW03_ObjectLocated> aoObjectsLocated;
            HW03_ObjectLocated oLocatedObject;
            int i;
            ...
            if (oEventToHandle.getEventType() == HW03_ObjectEvent.EventType.USER_INPUT) {
                if (oEventArguments.get("sKey").equals("START_RECORDING") && oEventArguments.get("bPressed").equals("TRUE")) {
                    O_EVENT_MANAGER.raiseEventReplay(HW03_Replay.ReplayEventType.RECORD_START);
                }
                else if (oEventArguments.get("sKey").equals("STOP_RECORDING") && oEventArguments.get("bPressed").equals("TRUE")) {
                    O_EVENT_MANAGER.raiseEventReplay(HW03_Replay.ReplayEventType.RECORD_END);
                }
                    ...
                    O_EVENT_MANAGER.raiseEventReplay(HW03_Replay.ReplayEventType.REPLAY_START);
                }
            }
            
            // REPLAY
            if (oEventToHandle.getEventType() == HW03_ObjectEvent.EventType.REPLAY) {
                if (oEventArguments.get("nReplayEventType") == HW03_Replay.ReplayEventType.RECORD_START) {
                    
                    // Save state
                    nCurrentState = ReplayState.RECORDING;
                    
                    // Start (or restart - to support multiple replays) timer
                    if (HW03_Time.doesTimerExist(S_TIMER_RECORDING) == true) {
                        HW03_Time.restartTimer(S_TIMER_RECORDING);
                    }
                    else {
                        HW03_Time.startTimer(S_TIMER_RECORDING);
                    }
                     ...
                     */
                    aoObjectsLocated = HW03_ObjectLocated.getLocatedObjects();
                ...
                else if (oEventArguments.get("nReplayEventType") == HW03_Replay.ReplayEventType.RECORD_END) {
                    
                    // Save state
                    nCurrentState = ReplayState.WAITING_TO_REPLAY;
                    
                    // Stop timer
                    HW03_Time.stopTimer(S_TIMER_RECORDING);
                    
                    // Pause the game during replay
                    O_EVENT_MANAGER.raiseEventGamePause(true);
                    
                }
                else if (oEventArguments.get("nReplayEventType") == HW03_Replay.ReplayEventType.REPLAY_START) {
                    ...
                    for (i = 0; i < aoObjectsLocated.size(); i++) {
                        ...
                        if (
                            ...
                            if (oLocatedObject instanceof HW03_ObjectRenderable) {
                                ((HW03_ObjectRenderable) oLocatedObject).setHiddenFlag(false);
                            }
                        }
                        else {
                            if (oLocatedObject instanceof HW03_ObjectRenderable) {
                                ((HW03_ObjectRenderable) oLocatedObject).setHiddenFlag(true);
                            }
                            else {
                ...
                else if (oEventArguments.get("nReplayEventType") == HW03_Replay.ReplayEventType.REPLAY_END) {
