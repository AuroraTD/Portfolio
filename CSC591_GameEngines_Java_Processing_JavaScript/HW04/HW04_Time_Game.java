...
// CLASS DEFINITION
public class HW04_Time_Game extends HW04_Time {
    
    // Required for serializable class
    private static final long serialVersionUID =            1L;
    
    // Private variables
    private static HW04_Time_Game oInstance =               null;
    ...
    /*********************************************************************************************************
     * FUNCTION:        HW04_Time_Game Constructor
     *
     * AUTHOR:          attiffan    Aurora T. Tiffany-Davis
     *
     ...
     * RETURNS:         None
     *********************************************************************************************************/
    public static HW04_Time_Game getInstance () {
        if (oInstance == null) {
            oInstance = new HW04_Time_Game();
        }
        return oInstance;
    }
    ...
    void start () {
        ...
        try {
            ...
            if (isStarted() == true) {
                ...
                // Start the game time
                nTimeGame_ms =              0;
                nTimeOffset_ms =            HW04_Time_Real.getInstance().getTime();
                nTimeGame_ms =              HW04_Time_Real.getInstance().getTime() - nTimeOffset_ms;
                nTimeGameLastResume_ms =    nTimeGame_ms;
                nTimeRealLastResume_ms =    HW04_Time_Real.getInstance().getTime();
                // Note that the game time has started
                HW04_EventManager.getInstance().raiseEventAdmin("Game Time Started");
            }
    ...
    void pause () {
        ...
        try {
            ...
            // Pause game time
                ...
                nTimeRealLastPause_ms = HW04_Time_Real.getInstance().getTime();
            }
    ...
    double getTime() {
        ...
        try {
            ...
            if (nTimeGame_ms < 0) {
                ...
                nTimeGame_ms = nTimeGameLastResume_ms + (HW04_Time_Real.getInstance().getTime() - nTimeRealLastResume_ms);
                nTime_ms = nTimeGame_ms;
            }
